...
function VerifierAge() {
  ...
  if (!walletId) {
    return <main className="blauth-consent"><nav className="blauth-register-nav" aria-label="Age verifier navigation"><Link className="blauth-brand" to="/" aria-label="BLAuth home"><span className="blauth-brand-mark">P</span><span>PehchanChain</span></Link></nav><section className="blauth-consent-empty" aria-labelledby="age-empty-title"><p className="blauth-eyebrow"><span /> Derived disclosure</p><h1 id="age-empty-title">No backend<br /><em>wallet found.</em></h1><p>Complete local face verification to register your backend wallet before checking age eligibility.</p><Link className="blauth-button blauth-button-primary" to="/register">Create Identity <span>→</span></Link></section></main>;
  }
  ...
  if (result) {
    ...
    return (
      <main className="blauth-consent">
        <nav className="blauth-register-nav" aria-label="Age verifier navigation"><Link className="blauth-brand" to="/" aria-label="BLAuth home"><span className="blauth-brand-mark">P</span><span>PehchanChain</span></Link><span className="blauth-nav-status"><i /> Derived data only</span></nav>
